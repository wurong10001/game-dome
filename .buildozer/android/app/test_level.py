...
class WaterScene(Widget):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.camera = Camera(position=[0, 0, 5], rotation=[0, 0, 0])
        self.world_texture = CoreImage(os.path.join('assets', 'water.tga')).texture

        with self.canvas:
            Color(1, 1, 1, 1)
            self.rect = Rectangle(texture=self.world_texture, pos=self.pos, size=(10000, 10000))

        self.bind(pos=self.update_rect, size=self.update_rect)

    def update_rect(self, *args):
        self.rect.pos = self.pos
        self.rect.size = (10000, 10000)
...
class TestLevel(Widget):
    ...
    def update(self, dt):
        # This would be where you'd render the frame using the renderer and camera
        pass
